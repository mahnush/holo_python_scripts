...
fileList = open('/home/mhaghigh/nc_file_in_outplume/holuhraun_newv_icon/modis_holo/7sep/file_list.txt', 'r+')
fileList_geo = open('/home/mhaghigh/nc_file_in_outplume/holuhraun_newv_icon/modis_holo/7sep/g_file_list.txt', 'r+')
...
allLat_5km = []
allLon_5km = []
ipath = '/home/mhaghigh/nc_file_in_outplume/holuhraun_newv_icon/modis_holo/7sep/'
# name of variables
...
#reff_grid = ma.masked_where(reff_grid > 20., reff_grid)
#to test the effect of tau smaller than 4
tau_grid = ma.masked_where(tau_grid < 4, tau_grid)
reff_grid_test = reff_grid[ctp_grid > 273]
print(np.max(reff_grid_test))

# to compute Nd
nd_grid = (1.37e-5 * (tau_grid ** 0.5) * ((reff_grid * 1e-6) ** -2.5)) * 1e-6
nd_c_grid = 0.32 * (tau_grid) * ((reff_grid * 1e-6) ** -2)
...
ncout = Dataset('/home/mhaghigh/nc_file_in_outplume/holuhraun_newv_icon/modis_holo/'
                'geo_modis_data_7d.nc', mode="w", format='NETCDF4_CLASSIC')
...
nd_c_o = ncout.createVariable('nd_c', np.float32, ('lon', 'lat'))
re_o[:] = reff_grid[:]
tau_o[:] = tau_grid[:]
lwp_o[:] = lwp_grid[:]
nd_o[:] = nd_grid[:]
