...
path_c = '/home/mhaghigh/nc_file_in_outplume/holuhraun_newv_icon/test_resolution/'
path_p = '/home/mhaghigh/nc_file_in_outplume/holuhraun_newv_icon/test_resolution/'
...
icon_output_names_1sep = 'run_1sep_reso_do1.nc'
...
icon_output_names_1sep_per = 'run_1sep_reso_do2.nc'
...
omps_data = [omps_path_1sep]
model_per = [model_per_1sep]
model_con = [model_con_1sep]
modis_data = [modis_path_1sep]
...
def get_in_out(i):
    ...
    lat_fine = postpro_func.read_nc(model_con[i], 'lat')[50:1551]
    lon_fine = postpro_func.read_nc(model_con[i], 'lon')[50:4051]
    lat_fine_do2 = postpro_func.read_nc(model_per[i], 'lat')
    lon_fine_do2 = postpro_func.read_nc(model_per[i], 'lon')
    control_var_lando = postpro_func.read_nc(model_con[i], var_name_model[0])
    control_var_lando = control_var_lando[50:1551, 50:4051]
    print(np.shape(control_var_lando))
    print(lat_fine)
    print(lon_fine)
    print(np.shape(lat_fine))
    print(np.shape(lon_fine))
    ...
    perturbed_var = postpro_func.mask_land(perturbed_var_lando, lat_fine_do2, lon_fine_do2)
    modis_var_lando = postpro_func.read_nc(modis_data[i], var_name_modis[0])
    modis_var_lando = np.transpose(modis_var_lando)
    lat_modis = np.arange(50, 80.02, 0.02)
    lon_modis  = np.arange(-60, 20.02, 0.02)
    modis_var = postpro_func.mask_land(modis_var_lando, lat_modis, lon_modis)
    ...
    scale_interp_do2 = f(lat_fine_do2, lon_fine_do2)
    con_inside = control_var[scale_interp > 1.0]
    con_outside = control_var[scale_interp < 1.0]
    per_inside = perturbed_var[scale_interp_do2 > 1.0]
    per_outside = perturbed_var[scale_interp_do2 < 1.0]
...
axs0.hist(allvar_in_per, bins=numbin, weights=weight(allvar_in_per) , histtype='step',
          linewidth=line_width, color='red', label='1km resolution '+lable_hist(allvar_in_per))

axs0.hist(allvar_in_con,  bins=numbin, weights=weight(allvar_in_con),  histtype='step',
          linewidth=line_width, color='blue', label='2km resolution '+lable_hist(allvar_in_con))
...
axs1.hist(allvar_out_per, bins=numbin, weights=weight(allvar_out_per),  histtype='step',
          linewidth=line_width, color = 'red', label='1 km resolution '+lable_hist(allvar_out_per))
axs1.hist(allvar_out_con, bins=numbin, weights=weight(allvar_out_con),  histtype='step',
          linewidth=line_width, color = 'blue', label='2 km resolution '+lable_hist(allvar_out_con))
